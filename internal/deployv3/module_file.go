...
func (m *fileModule) absent(tc TaskContext, args *fileArgs, info ops.RemoteFileInfo) ModuleResult {
	...
	result := runRemote(tc, "rm -rf -- "+shellQuote(args.Path))
...
func (m *fileModule) directory(tc TaskContext, args *fileArgs, info ops.RemoteFileInfo) ModuleResult {
	...
	if !info.Exists {
		...
		result := runRemote(tc, "mkdir -p -m "+mode+" -- "+shellQuote(args.Path))
		if result.Status != batch.StatusOK {
			return result
		}
		changed = true
		...
		result := runRemote(tc, "chmod "+args.Mode+" -- "+shellQuote(args.Path))
		if result.Status != batch.StatusOK {
			return result
		}
		changed = true
	...
	if args.Owner != "" || args.Group != "" {
		...
		result := runRemote(tc, "chown "+shellQuote(owner)+" -- "+shellQuote(args.Path))
		if result.Status != batch.StatusOK {
			return result
		}
		changed = true
...
func (m *fileModule) regularFile(tc TaskContext, args *fileArgs, info ops.RemoteFileInfo) ModuleResult {
	...
	if !info.Exists {
		...
		result := runRemote(tc, "touch -- "+shellQuote(args.Path))
		if result.Status != batch.StatusOK {
			return result
		}
		changed = true
	...
	if args.Owner != "" || args.Group != "" {
		...
		result := runRemote(tc, "chown "+shellQuote(ownerSpec(args.Owner, args.Group))+" -- "+shellQuote(args.Path))
		if result.Status != batch.StatusOK {
			return result
		}
		changed = true
...
func (m *fileModule) link(tc TaskContext, args *fileArgs, info ops.RemoteFileInfo) ModuleResult {
	...
	if info.Exists && !info.IsLink {
		if result := runRemote(tc, "rm -f -- "+shellQuote(args.Path)); result.Status != batch.StatusOK {
			return result
		}
	}
	result := runRemote(tc, "ln -sfn "+shellQuote(args.Src)+" "+shellQuote(args.Path))
	if result.Status != batch.StatusOK {
		return result
	}
