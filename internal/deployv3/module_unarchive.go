...
func (m *unarchiveModule) Run(tc TaskContext, raw any) ModuleResult {
	...
	mkdir := runRemote(tc, "mkdir -p -- "+shellQuote(extractDir))
	...
	mkdirDest := runRemote(tc, "mkdir -p -- "+shellQuote(args.Dest))
	if mkdirDest.Status != batch.StatusOK {
		return mkdirDest
	}
	sync := runRemote(tc, "cp -a "+shellQuote(extractDir+"/.")+" "+shellQuote(args.Dest))
...
func (m *unarchiveModule) cleanup(tc TaskContext, tempDir string) {
	...
	runRemote(tc, "rm -rf -- "+shellQuote(tempDir))
}
...
func remoteTreeHashes(tc TaskContext, dir string) (map[string]string, error) {
	...
	result := runRemote(tc, "cd "+shellQuote(dir)+" && find . -type f -print0 | sort -z | xargs -0 "+name+" "+flag)
...
func archiveKind(path string) archiveKindInfo {
	...
	case strings.HasSuffix(path, ".tar.gz"), strings.HasSuffix(path, ".tgz"):
		return archiveKindInfo{
			...
			listCommand: func(p string) string {
				return "tar -tzf " + shellQuote(p)
			},
			extractCommand: func(p, dir string) string {
				return "tar -xzf " + shellQuote(p) + " -C " + shellQuote(dir)
			},
		}
	case strings.HasSuffix(path, ".zip"):
		return archiveKindInfo{
			extension: ".zip",
			listCommand: func(p string) string {
				return "unzip -Z1 " + shellQuote(p)
			},
			extractCommand: func(p, dir string) string {
				return "unzip -q " + shellQuote(p) + " -d " + shellQuote(dir)
			},
		}
	default:
		return archiveKindInfo{}
