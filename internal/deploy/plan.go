...
func loadFile(path string) (*File, error) {
	...
	if err := yaml.NewDecoder(bytes.NewReader(data)).Decode(&header); err != nil {
		return nil, fmt.Errorf("解析 deploy 配置 %s 失败: %w", path, err)
	}
	if header.Version == 2 {
		return nil, fmt.Errorf("%s 使用已移除的 Deploy v2（v6.1.0 起不再支持）；请改写为 version: 3 playbook", path)
	}
	var file File
...
func validatePlayShape(play Play, selector TargetSelector) error {
	if strings.TrimSpace(play.Name) == "" {
		return fmt.Errorf("play 名称不能为空")
	}
	if selector.Empty() {
		return fmt.Errorf("play %q 的 hosts 不能为空（可在命令行用 --host/--tag/--all 覆盖）", play.Name)
	}
	if len(play.Tasks) == 0 {
...
func validateTask(task Task, vars Vars) error {
	...
	if task.When != "" {
		if err := ParseWhen(task.When); err != nil {
			return err
		}
	}
	if task.Module != "" {
