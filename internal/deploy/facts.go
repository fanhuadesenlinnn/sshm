...
// gatherFacts collects minimal host facts, caching them for factsTTL unless
// writeCache is false (for example during --check, which must stay read-only).
func gatherFacts(ctx context.Context, host config.Host, executor ops.Executor, connectTimeout time.Duration, cacheDir string, writeCache bool) (Vars, error) {
	cachePath := filepath.Join(cacheDir, factCacheName(host)+".json")
...
// factCacheName keys the cache by the stable host ID when available, falling
// back to the alias, so distinct aliases can never collide on one file.
func factCacheName(host config.Host) string {
	key := host.Alias
	if host.ID != "" {
		key = host.ID
	}
	return safeFactName(key)
}
