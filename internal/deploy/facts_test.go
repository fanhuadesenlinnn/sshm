...
func TestGatherFacts(t *testing.T) {
	...
	facts, err := gatherFacts(context.Background(), host, factsExecutor, 1000000000, cacheDir, true)
	if err != nil {
...
func (f *factsFakeExecutor) DialTCP(context.Context, config.Host, string, time.Duration) (net.Conn, error) {
