...
func (app *App) printInteractiveHelp() {
	...
	fmt.Printf("    %-14s %-24s %s\n", "find-con, f, pick", "打开可搜索主机选择器", "")
	fmt.Printf("    %-14s %-24s %s\n", "ping, p", "测试连通性", "ping [别名|ID]")
	fmt.Printf("    %-14s %-24s %s\n", "exec, x", "远程执行命令", "x [选项] <别名|ID> [选项] -- <命令>")
	fmt.Printf("    %-14s %-24s %s\n", "exec-tag, xt", "按标签执行命令", "xt [选项] <标签> [选项] -- <命令>")
	fmt.Println("    -- 后的内容会作为远程命令原样传递；复杂命令建议始终使用 --")
	fmt.Printf("    %-14s %-24s %s\n", "deploy", "批量部署工作流", "deploy <子命令>")
	fmt.Println()
	fmt.Println("  认证与密钥")
	fmt.Printf("    %-14s %-24s %s\n", "key, k", "进入托管密钥中心", "")
	fmt.Printf("    %-14s %-24s %s\n", "passwd", "设置 SSH 密码", "passwd <目标...|--tag 标签|--all>")
	fmt.Printf("    %-14s %-24s %s\n", "forget-pass", "删除已存密码", "forget-pass <目标...|--tag 标签|--all>")
