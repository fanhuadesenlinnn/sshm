...
> 版本说明：产品发布版本是 `v6.0.6`，Go module 是 `github.com/fanhuadesenlinnn/sshm/v6`，主配置与 Deploy 配置的 schema 都是 `version: 2`。
...
```bash
go install github.com/fanhuadesenlinnn/sshm/v6@v6.0.6
```

如果 `proxy.golang.org` 访问较慢，可临时指定国内代理：

```bash
GOPROXY=https://goproxy.cn,direct go install github.com/fanhuadesenlinnn/sshm/v6@v6.0.6
```

PowerShell：

```powershell
$env:GOPROXY = "https://goproxy.cn,direct"; go install github.com/fanhuadesenlinnn/sshm/v6@v6.0.6
```
